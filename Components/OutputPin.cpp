...
Connection** OutputPin::GetConnections(int &count) const
{
	count = m_Conn;
	if (m_Conn == 0) return NULL;
	Connection ** arrconnections = new Connection*[m_Conn];
	for (int i = 0; i < m_Conn; i++)
	{
		arrconnections[i] = m_Connections[i];
	}
	return arrconnections;
}

bool OutputPin::GetConnectionStat()
{
	if (m_Conn == 0) return false;
	return true;
}
void OutputPin::DeleteConnection(Connection *pC)
{
	for (int i = 0; i < m_Conn; i++)
	{
		if (m_Connections[i] == pC)
		{
...
void OutputPin::SetCompID(int ID)
{
	CompID = ID;
}

int OutputPin::GetCompID() const
{
	return CompID;
}